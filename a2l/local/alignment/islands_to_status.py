...
for k in range(len(res_ref_hyp_contents)):#,res_hyp_ref_contents):
	...
	text_start_index, text_end_index = list(map(int, i.split(' ')))
	ctm_start_index, ctm_end_index = list(map(int, j.split(' ')))
	ctm_begin_segment_id=ctm_contents[ctm_start_index].strip().split(' ')[0]
	ctm_end_segment_id=ctm_contents[ctm_end_index].strip().split(' ')[0]; print(segments_time_map[ctm_begin_segment_id])
	...
	if(make_complete_status and (k+1)<len(res_ref_hyp_contents)):
		next_text_start_index,_=list(map(int, res_ref_hyp_contents[k+1].split(' ')))
		next_text_start_index=next_text_start_index+text_initial_index
		next_ctm_start_index,_=list(map(int, res_hyp_ref_contents[k+1].split(' ')))
